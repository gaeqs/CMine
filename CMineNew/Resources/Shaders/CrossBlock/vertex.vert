...
void main () {
    ...
    data.fragNormal = mat3(transpose(inverse(model))) * normal;

    vec2 minT = textureArea.xy;
    vec2 maxT = textureArea.zw;
    vec2 size = maxT - minT;

    data.fragTexCoord = minT + texturePosition * size;
    ...
    data.fragColorFilter = vec4(r, g, b, a) / 255.0;
    data.fragLight = max(blockLight, sunlight * 0.8 + sunlight *  max(0, dot(-data.fragNormal, sunlightDirection)) * 0.2);
}
